    ...
    <section className="space-y-5">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <article className="rounded-lg border border-slate-200 bg-slate-50 p-4">
          <h3 className="font-semibold text-slate-900">Profile Foundation</h3>
          {loading ? (
            <p className="mt-2 text-sm text-slate-600">Loading profile data...</p>
          ) : (
            <p className="mt-2 text-sm text-slate-700">
              Status: <span className="font-medium">{profile?.profile_status || 'incomplete'}</span>
            </p>
          )}
        </article>
